...
def plan_letters(letter_string: str):
    ...
    letters.add('takeoff', color='blue', duration=5, led_delay=0)
    # for theta in np.linspace(0, 2*np.pi, 5)[1:]:
    #             letters.add('takeoff', color='gold', duration=3, led_delay=0, angle=theta)
...
trajectories, data = plan_letters('A L A')

with open('scripts/data/short_form.json', 'w') as f:
    json.dump(data, f)

tgen.plot_trajectories(trajectories)
plt.show()

tgen.gazebo_animate_trajectories("short_test.world",trajectories)
