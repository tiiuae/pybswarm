...
formations['S_1'] = Formation(
    ...
    order=[0, 1, 2, 3])
plot_formation(formations['S_1'], 'S_1')


#%% S_2
formations['S_2'] = Formation(
    points=scale_formation(np.array([
        [-0.5, -1, 1],
        [-0.5, 0, 1],
        [0.5, 0, 1],
    ...
    order=[0, 1, 2, 3])
plot_formation(formations['S_2'], 'S_2')

#%% S_3
formations['S_3'] = Formation(
    points=scale_formation(np.array([
        [-0.5, -1.5, 1],
        [-0.5, 0.5, 1],
        [0.5, 0, 1],
    ...
    order=[3, 0, 1, 2])
plot_formation(formations['S_3'], 'S_3')
...
def plan_letters(letter_string: str):
    ...
    for i, letter in enumerate(letter_string.split(' ')):
        ...
        letters.add(letter, color='gold', duration=3, led_delay=0)
        
    letters.add('takeoff', color='blue', duration=5, led_delay=0)
    
    letters.add('takeoff', color='blue', duration=10, led_delay=0)

    trajectories = letters.plan_trajectory(origin=np.array([0, 0, 1]))
...
trajectories, data = plan_letters('S_1 S_2 S_3')

with open('scripts/data/stag_form.json', 'w') as f:
    json.dump(data, f)

tgen.plot_trajectories(trajectories)
plt.show()

tgen.gazebo_animate_trajectories("stag.world",trajectories)
...
tgen.animate_trajectories('test_formation.mp4', trajectories, fps=5)
